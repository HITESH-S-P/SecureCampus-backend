...
const freeSubjectList = async (req, res) => {
    try {
        let subjects = await Subject.find({ sclassName: req.params.id, manager: { $exists: false } });
        if (subjects.length > 0) {
            res.send(subjects);
        } else {
            res.send({ message: "No subjects found" });
...
const getSubjectDetail = async (req, res) => {
    try {
        ...
        if (subject) {
            ...
            subject = await subject.populate("manager", "name")
            res.send(subject);
        }
        else {
...
const deleteSubject = async (req, res) => {
    try {
        ...
        // Set the teachSubject field to null in managers
